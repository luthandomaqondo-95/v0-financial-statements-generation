...
interface TiptapEditorContextType {
    activeEditorRef: RefObject<Editor | null> | null;
    setActiveEditor: (ref: RefObject<Editor | null> | null) => void;
...
const TiptapEditorContext = createContext<TiptapEditorContextType | null>(null);

const MAX_HISTORY = 50;

export function TiptapEditorProvider({ children }: { children: ReactNode }) {
    const [activeEditorRef, setActiveEditorRef] = useState<RefObject<Editor | null> | null>(null);
    ...
    const setActiveEditor = useCallback((ref: RefObject<Editor | null> | null) => {
...
export function useTiptapEditorContext() {
    ...
    if (!context) {
        throw new Error("useTiptapEditorContext must be used within a TiptapEditorProvider");
    }
    return context;
}