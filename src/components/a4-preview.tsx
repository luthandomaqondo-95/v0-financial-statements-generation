...
export function A4Preview({ content, onPageChange }: A4PreviewProps) {
  ...
  const A4_WIDTH = 794 // pixels (8.27in * 96dpi)
  const A4_HEIGHT = 1123 // pixels (11.69in * 96dpi)
      ...
      {pages.map((pageContent, index) => (
        ...
        >
          ...
          <div className="absolute bottom-8 left-0 right-0 flex justify-center">
            <span className="text-xs text-gray-500 font-serif">Page {index + 1}</span>
          </div>
        </div>
      ))}
    </div>
